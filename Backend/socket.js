...
function initializeSocket(server) {
  if (io) {
    console.log('Socket.io already initialized on port 4000');
    return io;
  }
  ...
  io = socketIo(server, {
    cors: {
      origin: [
        'http://localhost:5173',
        'https://rt5gcc3t-5173.inc1.devtunnels.ms',
  ...
  console.log('Socket.io initialized on port 4000:', io ? 'Yes' : 'No');
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('end-ride', async (data) => {
      ...
      try {
        ...
        if (!ride) {
          return socket.emit('error', { message: 'Ride not found or not eligible to end' });
        }
        ...
        await captainModel.updateOne(
          ...
          {
            $push: { dailyStats: { date: today, earnings: ride.fare, tripsCompleted: 1 } },
            $inc: { totalRides: 1 } // Add this
          }
        );
        await captainModel.updateOne(
          ...
          {
            $inc: {
              'dailyStats.$.earnings': ride.fare,
              'dailyStats.$.tripsCompleted': 1,
              totalRides: 1 // Add this
            }
          }
        );
        ...
        io.to('admin-room').emit('ride-completed', {
          rideId: ride._id,
        ...
        // Update user's totalRides
        if (ride.serviceType === 'taxiPool') {
          await Promise.all(
            ride.users.map(async (userEntry) => {
              await userModel.updateOne(
                { _id: userEntry.userId },
                { $inc: { totalRides: 1 } }
              );
            })
          );
        ...
        // Emit service stats update to admins
...
function sendMessageToSocketId(socketId, messageObject) {
  ...
  if (io) {
    ...
    console.error('Socket.io not initialized on port 4000. Message not sent.');
  }
}
