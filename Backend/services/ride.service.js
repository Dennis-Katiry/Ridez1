...
module.exports.createRide = async ({ user, pickup, destination, vehicleType, serviceType = 'solo', scheduledTime, io }) => {
  ...
  const userData = await userModel.findById(user); // Assuming user is the user ID or user object
  if (!userData) {
...
module.exports.getRideStatus = async ({ rideId, user }) => {
  ...
  const ride = await rideModel.findOne({ _id: rideId, user: user._id }).select('status');
  if (!ride) {
    throw new Error('Ride not found');
  }
