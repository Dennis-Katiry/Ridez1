...
module.exports.generateReport = async (req, res) => {
  try {
    ...
    doc.on('end', () => {
      ...
      res.setHeader('Content-Disposition', `attachment; filename=transport-service-report-${new Date().toISOString()}.pdf`);
      res.send(pdfData);
    });

    doc.fontSize(20).text('Ridez Transport Service Report', { align: 'center' });
    doc.moveDown();
